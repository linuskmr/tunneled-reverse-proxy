...
async fn main() -> Result<(), Box<dyn Error>> {
	...
	let (origin_proxy_reader, origin_proxy_writer) = origin_proxy.split();
	let origin_proxy_reader = BufReader::new(origin_proxy_reader);
	...
	tokio::spawn(async move {
		loop {
			...
			origin_proxy_client_send_channel.send(origin_proxy_client).await.context("Send inside socket to other thread").unwrap();
		}
	});

	let remote_client = TcpListener::bind(&cli_args.outside_addr).await.context(format!("Listening on {} for remote clients", &cli_args.outside_addr))?;
	...
	loop {
		...
		origin_proxy_writer.write_all(format!("{:?}\n", remote_client.peer_addr()?).as_bytes()).await.context("Sending outside client addr to server")?;
		origin_proxy_writer.flush().await.context("Sending outside client addr to server")?;
		
		let mut origin_proxy_client = origin_proxy_client_recv_channel.recv().await.ok_or(anyhow::anyhow!("No origin proxy client available"))?;
		...
		tokio::spawn(async move {
			...
			io::copy_bidirectional(&mut origin_proxy_client, &mut remote_client).await.context("Copying between remote client and origin proxy client")
		});
	}
}
