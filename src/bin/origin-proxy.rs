...
async fn main() -> Result<(), Box<dyn error::Error>> {
	...
	let remote_proxy_control_channel = TcpStream::connect(&cli_args.remote_proxy_addr)
		.await
		.context(format!("Connecting to proxy at {}", cli_args.remote_proxy_addr))?;
	...
	loop {
		...
		remote_proxy_control_channel
			.read_line(&mut remote_client_addr)
			.await
			.context("Read next proxied client addr from proxy")?;
		...
		tokio::spawn({
			...
			async move {
				...
				let mut remote_proxy = TcpStream::connect(&cli_args.remote_proxy_addr).await.context(format!(
					"Creating connection to proxy for proxied client at {}",
					&cli_args.remote_proxy_addr
				))?;

				log::debug!("Creating socket to origin server at {}", &cli_args.origin_server_addr);
				let mut origin_server = TcpStream::connect(&cli_args.origin_server_addr)
					.await
					.context(format!("Creating connection to local server at {}", &cli_args.origin_server_addr))?;

				log::debug!("Copying {:?} <-> {:?}", remote_proxy.peer_addr()?, origin_server.peer_addr()?);
				io::copy_bidirectional(&mut origin_server, &mut remote_proxy)
					.await
					.context("Copying between proxied client and local server")
			}
		});
	}
	Ok(())
